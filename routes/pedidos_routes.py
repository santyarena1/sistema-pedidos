...
def guardar_pedido():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            fecha_entrega = fecha_emision + timedelta(days=7)
            ...
            cur.execute("""
                ...
                RETURNING id
            """, (
                numero, data.get("nombre_cliente"), data.get("dni_cliente"),
                ...
                data.get("forma_envio"), float(data.get("costo_envio", 0))
            ))
            pedido_id = cur.fetchone()[0]

            for item in data.get("productos", []):
                # SOLUCIÓN: Se añade el estado por defecto al crear el producto
                cur.execute("""
                    INSERT INTO productos_pedido (pedido_id, producto, cantidad, precio_venta, estado_producto)
                    VALUES (%s, %s, %s, %s, 'FALTAN ENVIOS')
            ...
            for pago in data.get("pagos", []):
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """, (pedido_id, pago["metodo"], pago["monto"], pago.get("tipo_cambio"), pago.get("fecha")))
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            for pedido in pedidos:
                ...
                total_productos = sum(prod.get('precio_venta', 0) * prod.get('cantidad', 0) for prod in pedido['productos'])
                costo_envio = pedido.get('costo_envio', 0) or 0
                pedido['total_venta'] = total_productos + costo_envio
                
                # SOLUCIÓN: El cálculo de dólares se hace aquí, en el servidor.
                total_abonado_calculado = 0
                for pago in pagos:
                    monto = pago.get('monto', 0) or 0
                    if pago.get('metodo') in ['USD', 'USDT']:
                        tipo_cambio = pago.get('tipo_cambio', 1) or 1
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            """, (
                ...
                float(data.get("costo_envio", 0)), data.get("estado_general"), pedido_id
            ))

            # Se reconstruyen los productos y pagos
            cur.execute("DELETE FROM productos_pedido WHERE pedido_id = %s", (pedido_id,))
            for producto in data.get("productos", []):
                # SOLUCIÓN: Si el pedido general es "ENTREGADO", los productos también.
...
# --- Generar Constancia de Entrega (Sin cambios) ---
...
def generar_constancia_entrega(pedido_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            pedido = cur.fetchone()
            if not pedido: return "Pedido no encontrado", 404
            pedido = dict(pedido)
            cur.execute("SELECT * FROM productos_pedido WHERE pedido_id = %s", (pedido_id,))
            pedido["productos"] = [dict(row) for row in cur.fetchall()]
